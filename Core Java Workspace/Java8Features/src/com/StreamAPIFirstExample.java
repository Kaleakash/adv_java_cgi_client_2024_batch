...
class MyConsumer implements Consumer<Integer>{
	...
	System.out.println("Display value using consumer "+t);	
	}
}
public class StreamAPIFirstExample {
	...
	ll1.forEach(cc);			// using this logic retrieve record from list one by one 
	
	System.out.println("Retrieve the value using for each with consumer reference using lambda");
	ll1.forEach((Integer t)->System.out.println(t));
	}
